...
class RunStore:
    ...
    def write_run_manifest(self, manifest: RunManifest) -> RunManifest:
        ...
        canonical_manifest = manifest.copy(update={"run_root": str(run_root.resolve())})
        _write_json_atomic(run_root / "run_manifest.json", model_dump(canonical_manifest))
    ...
    def begin_attempt(
        ...
        *,
        ...
        self.write_run_manifest(
            manifest.copy(update={"current_attempt_id": attempt_id, "updated_at": timestamp})
        )
        return attempt
