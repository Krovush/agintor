...
def _run(spec: ValidatorSpec, payload: dict[str, Any]) -> ValidatorResult:
    ...
    if not expected_state:
        return ValidatorResult(
            ...
            status="abstain",
            authority_used="A0",
            health_status={"state_fixture_loaded": False, "side_effect_scan": True},
            observations={"reason": "missing_expected_state", "duplicate_side_effects": duplicate_side_effects, "policy_violations": list(policy_violations)},
        )
...
def _applicability(context: dict[str, Any]) -> float:
    if not context.get("stateful_service_fixture_available"):
        return 0.0
...
def family() -> ValidatorFamily:
    return ValidatorFamily(
        ...
        input_contract={"requires": ["final_state"], "sealed_requires": ["expected_state"]},
