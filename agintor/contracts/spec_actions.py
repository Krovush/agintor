...
class SpecAction(SpecActionModel):
    ...
    def validate_action(self) -> "SpecAction":
        ...
        assert_no_private_or_sealed_keys(self.patch)
        return self
...
def apply_spec_action(parent: RuntimeSpec, action: SpecAction) -> tuple[RuntimeSpec, SpecActionResult]:
    parent = validate_runtime_spec_payload(parent)
    action = SpecAction.model_validate(action)
    ...
    if action.action_type == "add_agent":
        ...
        for target_id in action.target_ids:
            agents, removed = _remove_by_id(list(validate_runtime_spec_payload(payload).agents), "agent_id", target_id)
            payload["agents"] = [agent.model_dump(mode="json") for agent in agents]
    ...
    child = validate_runtime_spec_payload(payload)
    ...
    action_ref = action_ref.model_copy(update={"child_spec_digest": child_digest}, deep=True)
    child = child.model_copy(update={"mutation_history": [*child.mutation_history[:-1], action_ref]}, deep=True)
...
def apply_spec_actions(parent: RuntimeSpec, actions: Sequence[SpecAction]) -> tuple[RuntimeSpec, list[SpecActionResult]]:
    current = validate_runtime_spec_payload(parent)
