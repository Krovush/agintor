...
from agintor.utils import jaccard, lexical_overlap, mean
...
class TopologyPolicy:
    ...
    def score_agent(self, ctx, agent, child_spec: ChildSpec) -> float:
        ...
        reusefit = lexical_overlap(" ".join(child_spec.required_capabilities), " ".join(agent.capability_set + agent.symbol_set))
    ...
    def propose_children(self, ctx, frame, operations: Sequence[Any]) -> list[ChildSpec]:
        ...
        for index, op in enumerate(operations):
            ...
            required_capabilities = ["memory"] if op.kind == "memory_lookup" else ["tooling" if op.kind == "generated_expression" else "arithmetic"]
            children.append(
                ChildSpec(
                    child_id=child_id,
                    role=role,
    ...
    def select_workers(self, ctx, frame, operations: Sequence[Any]) -> list[dict[str, Any]]:
        ...
        candidates = [
            ...
            {"worker_id": "w1", "instruction": "Reverse order plan", "op_ids": list(reversed(op_ids)), "predicted_solve": 0.58, "tool_scope": ctx.state.visible_tool_names, "agent_id": "root"},
            {"worker_id": "w2", "instruction": "Dependency-first plan", "op_ids": sorted(op_ids, key=lambda op_id: 0 if any(op.op_id == op_id and op.dependencies for op in operations) else 1), "predicted_solve": 0.55, "tool_scope": ctx.state.visible_tool_names, "agent_id": "root"},
        ]
        selected = []
        selected_ids = set()
        while len(selected) < min(self.K_MAX, len(candidates)):
            ...
            for worker in candidates:
                ...
                for existing in selected:
                    overlap = jaccard(worker["op_ids"], existing["op_ids"])
