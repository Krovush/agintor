...
- Long-term memory must become a durable, versioned knowledge graph with explicit write history, contradiction handling, and retrieval diagnostics while preserving the current exact-symbol and exact-path priority rules.
...
- Own the persistence contract for runtime host state, checkpoint payloads, replay/provenance storage, short-term graph durability, and long-term memory durability.
...
- Keep runtime scheduling, checkpoint emission timing, branch semantics, and request execution flow aligned with Workstream 2. This workstream owns the stored state model and recovery path, not the scheduler policy.
- Keep export bundling and CLI packaging aligned with Workstream 1. This workstream owns durable state formats and asset boundaries, not the user-facing export contract.
- Keep tool-runtime internals, async job lifecycle policy, sandbox implementation, and provider/runtime environment semantics aligned with Workstream 5. This workstream may persist their state and fingerprints, but it does not own those subsystems' behavioral logic.
- Keep factory archive persistence, evaluation restart, and long-running search orchestration aligned with Workstream 4. This workstream is runtime-state durability, not factory-search durability.
...
- `agintor/shell.py` already centralizes runtime-owned state in `FixedShell`: short-term memory, long-term memory, message board, open-handle table, predictors, agent pool, sandbox manager, tool registry, tool executor, and trace writing.
- `FixedShell.reset_for_task()` already enforces the intended long-term memory boundary: memory resets per task unless transfer-scored execution keeps the same episode scope.
- `FixedShell.validate_invariants()` already enforces open-handle integrity, short-term raw-output reachability, and long-term memory leakage rules.
- `agintor/memory_graph.py` already provides the right core semantics for short-term memory: append-only nodes and edges, summary replacement with backlinks, and hidden-node reachability validation.
- `LongTermGraph` already supports the required node types and already ranks retrieval with exact symbol and exact path matches ahead of lexical and embedding-style similarity.
- `agintor/runner.py` already emits checkpoint summaries, promotes long-term memory candidates, records short-term graph artifacts and handles, and writes traces when `retain_artifacts=True`.
- The current durability model is still process-local:
...
- `Checkpoint` is still a summary-oriented runtime object. It is not yet a restartable persisted state envelope.
- `EnvironmentFingerprint` exists in schema vocabulary but is not yet extracted from real runtime execution.
- The existing tests already cover important baseline invariants: short-term backlinks, exact-first retrieval, open-handle validation, long-term memory scope resets, and checkpoint contract shape.
...
- Add serializable models for:
  ...
  `CheckpointEnvelope`,
  and any required frame or branch snapshot objects that Workstream 2 introduces.
...
- Define versioned storage schemas up front so future migrations can reject incompatible checkpoint data cleanly instead of loading partially.
...
- Keep checkpoint emission boundaries owned by Workstream 2, but make the storage and recovery path complete enough that those checkpoints become restartable rather than summary-only.
...
- Keep predictor and tool-registry semantics owned by Workstream 5, but require those subsystems to offer storage-safe snapshot handles so runtime durability does not depend on their private layout.
...
- Make retained replay data explicit and opt-in where appropriate so routine evaluation can still stay lightweight.
...
`Exit gate:` long-term memory survives process restart, preserves versioned knowledge history, and remains inspectable and scope-correct across task and episode boundaries.
...
- Extract `EnvironmentFingerprint` nodes from real execution facts:
  ...
  and other non-secret execution facts exposed by Workstream 5.
...
- Use persisted environment fingerprints during recovery to detect incompatible resume attempts, such as a missing sandbox artifact, changed backend, or unavailable tool runtime.
- Record recovery outcomes explicitly:
  resumed,
  resumed with degraded handle recovery,
  failed closed because state was incompatible,
...
1. `FixedShell`, `RuntimeState`, short-term memory, long-term memory, message-board state, and open-handle state can all round-trip through persisted snapshots and still satisfy existing invariants.
2. A runtime interrupted after checkpoint emission can reload from the persisted checkpoint, rebuild live runtime state, and continue or fail closed with a precise recovery reason.
3. Retained runs leave behind durable provenance artifacts that support replay queries and normalized trace or graph diffs from the workspace.
4. Long-term memory persists across process restart when it should, resets when task or episode boundaries require it, and records contradiction or tombstone history without losing provenance.
5. Environment fingerprints are captured from real execution and are available in persisted recovery and replay records without leaking secrets.
