...
class OracleCompiler:
    ...
    def compile(self, goal: GoalSpec | dict[str, Any], runtime_spec: RuntimeSpec | dict[str, Any] | None = None, *, prior_ledgers: Sequence[dict[str, Any]] = ()) -> OraclePackage:
        ...
        spec_obj = RuntimeSpec.model_validate(runtime_spec) if runtime_spec is not None and not isinstance(runtime_spec, RuntimeSpec) else runtime_spec
        ...
        selected_families = self.registry.select(context, minimum_score=0.1, limit=8)
        ...
        package = OraclePackage(
            ...
            qa_report_ref="",
            frozen=True,
            metadata={"compiler_id": self.config.compiler_id, "selected_families": [family.family_id for family in selected_families]},
        )
    ...
    def _goal_text(goal: GoalSpec) -> str:
        parts = [str(getattr(goal, "prompt", "") or ""), str(getattr(goal, "objective", "") or ""), str(getattr(goal, "description", "") or "")]
        return "\n".join(part for part in parts if part.strip()) or str(goal.model_dump(mode="json"))
    ...
    def _context(self, goal: GoalSpec, runtime_spec: RuntimeSpec | None, prior_ledgers: Sequence[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "domain_trading": any(word in text for word in ["trading", "trade", "stock", "portfolio", "alpha", "pnl", "market"]),
            "domain_repo": any(word in text for word in ["repo", "patch", "code", "bug", "test"]),
            "domain_service": any(word in text for word in ["service", "api", "state", "database", "workflow"]),
            "domain_consent": any(word in text for word in ["consent", "authorization", "permission", "side effect"]),
        }
