...
class OracleTask(OracleModel):
    ...
    def public_task(self) -> BenchmarkTask:
        return runtime_visible_benchmark_task(self.benchmark_task)
...
class OraclePackage(OracleModel):
    schema_version: Literal["agintor.oracle_package.v1"] = "agintor.oracle_package.v1"
    package_id: str
...
def oracle_public_projection(package: OraclePackage | dict[str, Any]) -> dict[str, Any]:
                ...
                "tasks": [
                    {
                        ...
                        "benchmark_task": runtime_visible_benchmark_task(task.benchmark_task).model_dump(mode="json", exclude_none=True),
    ...
    return {
        ...
        "evidence_contract": pkg.evidence_contract.model_dump(mode="json", exclude_none=True),
        ...
        "leakage_policy": pkg.leakage_policy.model_dump(mode="json", exclude_none=True),
...
def oracle_public_view_hash(package: OraclePackage | dict[str, Any]) -> str:
    return stable_hash("agintor.oracle.public.v1", oracle_public_projection(package))


def oracle_sealed_view_hash(package: OraclePackage | dict[str, Any]) -> str:
    return stable_hash("agintor.oracle.sealed.v1", oracle_sealed_projection(package))
...
def oracle_package_hash(package: OraclePackage | dict[str, Any], *, assume_projection_hashes: tuple[str, str] | None = None) -> str:
    ...
    return stable_hash("agintor.oracle.package.v1", payload, public_hash, sealed_hash)
...
def oracle_runtime_visible_tasks_by_partition(package: OraclePackage, partition: str) -> list[BenchmarkTask]:
    return [runtime_visible_benchmark_task(task) for task in oracle_tasks_by_partition(package, partition)]
